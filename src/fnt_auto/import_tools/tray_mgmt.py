...
class TraySectionsImporter(ItemsImporter):        

    def __init__(self, fnt_api: FntAsyncAPI) -> None:
        super().__init__(fnt_api.tray_mgmt.tray_section)
        self.fnt_api = fnt_api

    async def initialize(self):
        self.fnt_items_exist = utils.to_dict(await self.fnt_api.tray_mgmt.tray_section.get_all(), key='id')

    async def _collect_items(self) -> list[TraySectionCreateReq]:
        return await self.collect_items()
    ...
    def _identify_key(self, new_item: TraySectionCreateReq) -> t.Optional[str]:
        return new_item.id